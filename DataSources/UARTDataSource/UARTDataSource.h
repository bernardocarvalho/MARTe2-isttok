...
/**
 * @brief A DataSource which implements a circular buffer implementation (using
 * a MemoryMapMultiBufferInputBroker) to a CRIOUARTSerial.
 *
 * @details This DataSource produces two signals. The first shall be of type
 * uint8 and will indicate if the data of the second signal is valid (e.g. it
 *  might have returned with a timeout). The second signal contains the data
 * read from the serial port.
 *
 * The frequency at which data arrives will vary on the number of events
 * received in a given period.
 *
 * An asynchronous thread reads from the UART and makes the data available to
 * the RealTimeThread.
 *
 * The configuration syntax is (names are only given as an example, but the size
 * and the exact number of signals shall be respected):
 *
 * <pre>
 *   +CRIOUART = {
 ...
 *       }
 *       Time = {
 *         Type = uint64 //Compulsory - time-stamp in nano-seconds at which the
 * data was read. NumberOfElements = 1
 *       }
 ...
 *       }
 *     }
 *     +TimeProvider = { //Optional. If not provided
 * HighResolutionTimestampProvider will be used. Class = TimestampProvider //A
 * class that provides the absolute time in ns. It shall inherit from
 * TimestampProvider.
 *     }
 *   }
 */
class UARTDataSource : public MemoryDataSourceI,
                       public MessageI,
                       public EmbeddedServiceMethodBinderT<UARTDataSource> {
public:
  ...
  /**
   * @brief Destructor. Stops the Embedded thread which reads from the
   * CRIOUARTSerial.
   */
   ...
   * @return true if the configuration respects all the rules described in the
   * class description.
   */
  virtual bool Initialise(StructuredDataI &data);

  /**
   * @see Checks that the signal properties are as described in the class
   * description.
   * @return true if the signal properties respect all the rules described in
  ...
  /**
   * @brief Callback function for the thread which asynchronously reads from the
   * UART.
   */
